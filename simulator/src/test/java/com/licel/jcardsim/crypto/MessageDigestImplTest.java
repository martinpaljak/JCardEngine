...
public class MessageDigestImplTest extends SimulatorCoreTest {
     ...
     * @param etalonDigest byte array contains etalon digest
     */
    private void testEngineDoUpdateFinal(MessageDigest engine, byte[] msg, byte[] etalonDigest) {
        byte[] digest = new byte[engine.getLength()];
    ...
    public void testSetInitialDigest() {
        ...
        for (MessageDigestImpl messageDigest : digests) {
            System.out.println("testSetInitialDigest() - " + messageDigest.getAlgorithm());

            byte[] digest = new byte[messageDigest.getLength()];
            byte[] etalonDigest = new byte[messageDigest.getLength()];
            messageDigest.reset();
            messageDigest.doFinal(inputData, (short) 0, (short) inputData.length, etalonDigest, (short) 0);
